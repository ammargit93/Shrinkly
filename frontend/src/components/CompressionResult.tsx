  ...
  return (
    <div className="border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 rounded-xl p-4 sm:p-6 space-y-4 sm:space-y-6 shadow-xs">
      ...
      <div className="text-center space-y-1">
        ...
        <h2 className="text-base sm:text-lg font-bold text-neutral-850 dark:text-neutral-100">
          {isAlreadyWithinLimit
            ? 'Image is already within your target size'
            : 'Compression complete'}
        </h2>
        <p className="text-xs text-neutral-450 dark:text-neutral-500 max-w-[480px] mx-auto">
          {isAlreadyWithinLimit
          ...
          {originalPreviewUrl && (
            <div className="inline-flex rounded-lg bg-neutral-100 dark:bg-neutral-800 p-0.5 text-xs">
              <button
                ...
                className={`px-2 sm:px-2.5 py-1 rounded-md font-medium transition-all cursor-pointer ${viewMode === 'compressed'
                  ? 'bg-white dark:bg-neutral-900 text-emerald-700 dark:text-emerald-400 shadow-xs'
                  : 'text-neutral-500 hover:text-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-200'
                  }`}
              >
                Compressed <span className="font-normal opacity-85">({formatBytes(result.compressedSize)})</span>
              </button>
              <button
                type="button"
                onClick={() => setViewMode('original')}
                className={`px-2 sm:px-2.5 py-1 rounded-md font-medium transition-all cursor-pointer ${viewMode === 'original'
                  ? 'bg-white dark:bg-neutral-900 text-emerald-700 dark:text-emerald-400 shadow-xs'
                  : 'text-neutral-500 hover:text-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-200'
                  }`}
              >
                Original <span className="font-normal opacity-85">({formatBytes(result.originalSize)})</span>
              </button>
            </div>
          )}
        </div>

        <div className="relative rounded-xl border border-neutral-200 dark:border-neutral-800 bg-neutral-100/50 dark:bg-neutral-950/50 p-3 sm:p-4 flex items-center justify-center min-h-[160px] max-h-[260px] sm:max-h-[320px] overflow-hidden">
          <img
            key={viewMode}
            ...
            className="max-h-[240px] sm:max-h-[280px] max-w-full object-contain rounded-lg shadow-xs"
          />
          <span className="absolute bottom-2 left-2 inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-medium bg-black/70 text-white px-2 py-0.5 rounded backdrop-blur-xs">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 py-1">
        <div className="p-2.5 sm:p-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/40 border border-neutral-100 dark:border-neutral-800 text-center">
          <div className="text-[11px] sm:text-xs text-neutral-500 dark:text-neutral-400 font-medium uppercase tracking-wider">Original</div>
          <div className="text-sm sm:text-base font-bold text-neutral-900 dark:text-neutral-100 mt-0.5">
            {formatBytes(result.originalSize)}
          </div>
        </div>

        <div className="p-2.5 sm:p-3 rounded-lg bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 text-center">
          <div className="text-[11px] sm:text-xs text-emerald-600 dark:text-emerald-400 font-medium uppercase tracking-wider">Compressed</div>
        ...
        <div className="p-2.5 sm:p-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/40 border border-neutral-100 dark:border-neutral-800 text-center">
          <div className="text-[11px] sm:text-xs text-neutral-500 dark:text-neutral-400 font-medium uppercase tracking-wider">Reduction</div>
          <div className="text-sm sm:text-base font-bold text-neutral-900 dark:text-neutral-100 mt-0.5">
        ...
        <div className="p-2.5 sm:p-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/40 border border-neutral-100 dark:border-neutral-800 text-center">
          <div className="text-[11px] sm:text-xs text-neutral-500 dark:text-neutral-400 font-medium uppercase tracking-wider">Format</div>
          <div className="text-sm sm:text-base font-bold text-neutral-900 dark:text-neutral-100 mt-0.5">
            {result.outputFormat}
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 sm:flex gap-2 sm:gap-2.5">
          {typeof navigator !== 'undefined' && navigator.clipboard && (
            <button
              type="button"
              onClick={handleCopy}
              className="flex items-center justify-center gap-1.5 py-2.5 sm:py-3 px-2 sm:px-3 text-xs sm:text-sm font-medium border border-neutral-200 hover:bg-neutral-50 active:scale-[0.98] dark:border-neutral-800 dark:hover:bg-neutral-800 text-neutral-700 dark:text-neutral-300 bg-white dark:bg-neutral-900 rounded-lg transition-all cursor-pointer"
          ...
          {onEditTarget && (
            <button
              ...
              className="flex items-center justify-center gap-1.5 py-2.5 sm:py-3 px-2 sm:px-3 text-xs sm:text-sm font-medium border border-neutral-200 hover:bg-neutral-50 active:scale-[0.98] dark:border-neutral-800 dark:hover:bg-neutral-800 text-neutral-700 dark:text-neutral-300 bg-white dark:bg-neutral-900 rounded-lg transition-all cursor-pointer truncate"
