...
export function ImageInfo({ info, onClear }: ImageInfoProps) {
  ...
  return (
    <div className="flex items-center justify-between p-3 sm:p-4 border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 rounded-xl shadow-xs">
      <div className="flex items-center gap-3 sm:gap-3.5 overflow-hidden flex-grow min-w-0 mr-2">
        <div className="relative flex-shrink-0">
          <img
            src={info.previewUrl}
            alt="Preview"
            className="h-12 w-12 sm:h-14 sm:w-14 object-cover border border-neutral-200 dark:border-neutral-800 rounded-lg bg-neutral-100 dark:bg-neutral-950"
          />
          {info.frameCount && info.frameCount > 1 && (
            <span className="absolute bottom-0.5 right-0.5 sm:bottom-1 sm:right-1 inline-flex items-center gap-0.5 px-1 py-0.2 text-[9px] font-semibold bg-black/75 text-white rounded backdrop-blur-xs">
      ...
      <button
        onClick={onClear}
        className="p-2.5 sm:p-2 -mr-1 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-lg text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200 transition-colors cursor-pointer flex-shrink-0"
